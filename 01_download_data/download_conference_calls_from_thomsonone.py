...
coords = {
'address_bar': (529, 35), 'screening_and_analysis': (665, 67), 'research': (306, 93), 
'contributor': (152, 605), 'refinitiv_streetevents': (177, 641), 
...
'xls_icon_multiple_pages_1': (1684, 763), 'xls_icon_multiple_pages_2': (1673, 763), 'xls_icon_one_page': (1714, 765),
'xls_save_more_options': (1317, 1016), 'xls_save_as': (1389, 994)
}
...
if __name__ == '__main__':
    ...
    parser.add_argument('relogin', help="whether to do the first login", type=int)
...
while date_start <= date_start_current:
    ...
    if restarted_process:
        ...
        date_start_current, date_end_current, date_start_current_str, date_end_current_str, date_range_current = get_new_time_window(date_start_current)
    
    enter_query(date_start_current, date_end_current, date_start_current_str, date_end_current_str, date_range_current)
    ...
    for current_page in range(number_of_pages_of_conf_calls):
        ...
        if os.path.exists(output_pdf_filepath) and os.path.exists(output_xls_filepath):
            ...
            mouse_click(coords['next_page'], 3) 
        ...
        if os.path.exists(output_pdf_filepath):
            ...
            mouse_click(coords['view_toc'], 10)
            expand_screen(2)
            ...
            while not(os.path.exists(output_pdf_filepath)):
                # Stop limit
                print('Saving pdf: attempt', attempt)
                if attempt > 5: 
                    print('Pausing process for 1 minute for manual fix')
                    time.sleep(60)
                if attempt > 10: 
                ...
                save_pdf_hotkey(1)
                # Type the file name
                enter_string(str(output_pdf_filepath), 1)
        ...
        if os.path.exists(output_xls_filepath):
            ...
            while not(os.path.exists(output_xls_filepath)):
                ...
                if number_of_pages_of_conf_calls > 1:
                    mouse_click(coords['xls_icon_multiple_pages_1'], 2)
                    mouse_click(coords['xls_icon_multiple_pages_2'], 2)
                else:
                    mouse_click(coords['xls_icon_one_page'], 2)

                # Save -> save as
                mouse_click(coords['xls_save_more_options'], 1)
