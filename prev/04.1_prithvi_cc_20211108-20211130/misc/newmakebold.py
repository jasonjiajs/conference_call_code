...
#%%%
raw_data = openpyxl.load_workbook(r"cric1_newtotal_prithvi.xlsx")
...
row = 0 
col = 0
with open(r"keyterms.txt", "r", encoding="utf-8", errors="ignore") as f1:
...
#%%% overview
overview_file = xlsxwriter.Workbook(r"Paragraph Record_prithvi.xlsx")
...
for row_val in range(1,raw_data.max_row):
    if row_val%500==1:
        ...
        cur_dir = "2nd_entry_files/"
        cur_file_name = "2nd_entry_files/" + str(row_val//500+1) + ".xlsx"
        if not os.path.exists(cur_dir):
            os.mkdir(cur_dir)
                
        final_data = xlsxwriter.Workbook(cur_file_name)
