``` File has to run in Julia language```
 ```This code parse text Concference Calls (CC) to CSV Data Frame ```
 ```Code uses three input files: 1) *.XLS file with a list of CC per file and 2) *.txt file, which was converted from pdf file and 3) the original pdf file ```
 ```Code use information from XLS file about type of Anaylist to parse CC in different ways ```
 ``` The current folder has to contain: 1) List folder 2) CallScripts folder ```
 ``` The result will be located in CSV folder ```
 ``` to run code: 1) execute code with funciton before main code
                  2) set current folder in main code
                 3) exucute main code               ```
...
function parseCalls(filename)
    try
    #get list of CC
    dflist=parseHtmlXlsToDf("List/$filename.xls")
    dflist[!,:Call].=""
...
try
    #set current directory to the List folder (*.csv files)
    #*.pdf and *.txt files have to located in ../CallScripts" folder
    cd("C:/CC/ConfCall20201024-20210405")
    try
        mkdir("CSV")
    catch e
    end
