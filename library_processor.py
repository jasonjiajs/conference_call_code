###### Created by SIXUN TANG @20200315
###### This python file serves to generate political&non-political libraries from textbooks(parsed from pdf, raw input are in text files)
...
os.chdir("G:\\My Drive\\Booth\\Political Firms\\Programs\\ConferenceCall")

#### read political library and non-political library ######
#### change the txt file name to generate different library ###
p_lib_text = ''
with open("Training Library\\P_Lib.txt", "r", encoding="utf-8", errors="ignore") as p_library:
    for line in p_library.readlines():  ## read by lines
        if line=="\n" or line==" ": ### do not include extra \n lines
            continue
        if ".indd" in line:  #### lines of page break
            continue
        split_items = line.split("  ")  ### identify multi blocks
        while "" in split_items:
            split_items.remove("")
        if len(split_items)>=3:  ### these lines are typically not text of what we want (additional contents)
            continue
        for item in split_items: ### if the line splits into two parts and one part contains only number, this is also problematic
            new_item = item.split()
            if num_iden(punc_reader(item,True)) and len(new_item)==1:
                continue
        p_lib_text += punc_reader(line, True).lower() + "\n"   ### rid of punctuation, lowercase


print(p_lib_text[1:12000])
...
#### add tagger and then construct bigrams as well as word-property bigrams #### 
...
with open("political_bigrams.txt","w",encoding="utf-8",errors="ignore") as p_file:
